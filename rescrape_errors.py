...
for year in years:
    ...
    print 'scraping missing boxscore data from %d season...' %(year)
    ...
    columns = players.columns #necessary to preserve order when writing file later
    error_list = pd.DataFrame(columns = ['game_id', 'home_team', 'visit_team']) #create new error list

    print '%d errors to process from %d season...' %(len(games), year)
